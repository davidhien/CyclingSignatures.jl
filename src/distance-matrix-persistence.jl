"""
    trajectoryBarcode(::Val{:DistanceMatrix}, trajPoints, metric, fltThreshold, field=DEFAULT_FIELD)
...
The distance complex of a the points in `trajPoints` with respect to the `metric` is the filtered complex where 
- vertices are pairs `(i, j)` with `i < j`, filtered by `metric(trajPoints[:, i], trajPoints[:, j])` and 
...
"""
    vertex_to_essential_bar(node, trajPoints, metric; field=DEFAULT_FIELD)
...
The distance complex is specified by `trajPoints` and `metric`.
...
"""
    filtration_min_vertices_implicit_dm(trajPoints, metric, fltThreshold)

Computes a filtration-minimal vertex of each connected component of the distance complex specified by `trajPoints` and `metric` up to the filtration threshold `fltThreshold`.
This is implmented using a two pass annotation algorithm for images, the image being the upper right part of the distance matrix.
...
function dm_components_second_pass_implicit(cc::IntDisjointSets{Int}, cc_labels::Matrix{Int}, trajPoints, metric)
    smallestNodes = Dict{Int,Tuple{Int,Int}}()
