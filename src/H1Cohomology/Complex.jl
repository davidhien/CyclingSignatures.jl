...
"""
function one_skeleton(D::AbstractArray{T,2}, r) where T<:Number
    n = size(D, 1)
    cells = Dict{Int,Vector{Simplex}}()
    cells[0] = map(x -> Simplex([x]), 1:n)[:]
...
"""
function vr_incremental(X::AbstractArray{T,2}, d, r; maxdim=2) where T<:Number
    D = pairwise(d, X, dims=2) # can maybe be improved by only computing lower left part.
    return vr_incremental(D, r, maxdim=maxdim)
end
